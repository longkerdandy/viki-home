 ...
 */
public class HAPSRP6ServerSession extends SRP6Session {
   ...
   * counterparty (server or client) fails to respond within the specified time the session will be
   * closed. If zero timeouts are disabled.
   */
  public HAPSRP6ServerSession(final SRP6CryptoParams config, final int timeout) {

    super(timeout, new HAPSRP6Routines());  // Change to compatible with Homekit Accessory Protocol
    ...
    state = HAPSRP6ServerSession.State.INIT;
  ...
  /**
   * Creates a new server-side SRP-6a authentication session and sets its state to {@link
   * HAPSRP6ServerSession.State#INIT}. Session timeouts are disabled.
   *
   * @param config The SRP-6a crypto parameters configuration. Must not be {@code null}.
   */
  public HAPSRP6ServerSession(final SRP6CryptoParams config) {

    this(config, 0);
  }

  /**
   * Increments this SRP-6a authentication session to {@link HAPSRP6ServerSession.State#STEP_1}.
   *
   * <p>Argument origin:
   *
   * <ul>
   ...
   * @throws IllegalStateException If the mehod is invoked in a state other than {@link
   * HAPSRP6ServerSession.State#INIT}.
   */
  public BigInteger step1(final String userID, final BigInteger s, final BigInteger v) {
    ...
    // Check current state
    if (state != HAPSRP6ServerSession.State.INIT) {
    ...
    state = HAPSRP6ServerSession.State.STEP_1;
  ...
  /**
   * Increments this SRP-6a authentication session to {@link HAPSRP6ServerSession.State#STEP_1}
   * indicating a non-existing user identity 'I' with mock (simulated) salt 's' and password
   * verifier 'v' values.
   *
   * <p>This method can be used to avoid informing the client at step one
   * that the user identity is bad and throw instead a guaranteed general "bad credentials" SRP-6a
   * exception at step two.
   *
   * <p>Argument origin:
   *
   * <ul>
   ...
   * specified identity 'I': salt 's' and password verifier 'v' values.
   * </ul>
   *
   * @param userID The identity 'I' of the authenticating user. Must not be {@code null} or empty.
   * @param s The password salt 's'. Must not be {@code null}.
  ...
  /**
   * Increments this SRP-6a authentication session to {@link HAPSRP6ServerSession.State#STEP_2}.
   *
   * <p>Argument origin:
   *
   * <ul>
   ...
   * @throws IllegalStateException If the method is invoked in a state other than {@link
   * HAPSRP6ServerSession.State#STEP_1}.
   */
  public BigInteger step2(final BigInteger A, final BigInteger M1)
    ...
    // Check current state
    if (state != HAPSRP6ServerSession.State.STEP_1) {
    ...
    if (!srp6Routines.isValidPublicValue(config.N, A)) {
      throw new SRP6Exception("Bad client public value 'A'",
          SRP6Exception.CauseType.BAD_PUBLIC_VALUE);
    }
    ...
    state = HAPSRP6ServerSession.State.STEP_2;
    ...
    if (serverEvidenceRoutine != null) {
      ...
      // With default routine
      M2 = ((HAPSRP6Routines) srp6Routines).computeServerEvidence(digest, A, M1, S);
      digest.reset();
    }
   ...
   */
  public HAPSRP6ServerSession.State getState() {
